...
export const STAGES = ["ingest","cleanup","retarget","validate","export","deliver"];
...
export const STAGE_INFO = {
  ingest:{
    ...
    pattern:"Strategy Pattern",lesson:"09-production-capture-pipeline.html",code:"capture_pipeline/pipeline/runner.py"
  },
  cleanup:{
    title:"Cleanup Stage",
...
// Helper functions (ported from ui.js)
export function getExpAdapter(c) {
  return c.export.format === "gltf" ? "GLTFExportAdapter" : "FBXExportAdapter";
}

export function getDelAdapter(c) {
  var m = c.delivery.method;
  if (m === "perforce") return "PerforceDelivery";
  if (m === "nas") return "NASDelivery";
  if (m === "s3") return "S3Delivery";
  return "SFTPDelivery";
}

export function getDest(c) {
  var m = c.delivery.method;
  if (m === "perforce") return c.delivery.depot_path || "";
  if (m === "nas") return c.delivery.nas_path || "";
  if (m === "s3") return c.delivery.s3_bucket ? "s3://" + c.delivery.s3_bucket + "/" : "";
  if (m === "sftp") return c.delivery.sftp_host || "";
  return "";
}
