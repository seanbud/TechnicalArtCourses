...
import { CL, stadium_project, STAGES } from '../data/data';
...
export default function Inspector() {
  ...
  function renderContent() {
    if (inspectorTab === "data") {
      ...
      if (selectedFile && stadium_project[selectedFile]) {
        return (
          <div>
            <h4 className="inspector-title">{selectedFile.split('/').pop()}</h4>
            <div dangerouslySetInnerHTML={{ __html: highlightPy(stadium_project[selectedFile]) }} />
          </div>
        );
      }
    ...
    if (inspectorTab === "adapter") {
      const adapterFile = selectedFile && stadium_project[selectedFile] ? selectedFile : null;
      if (adapterFile) {
        return (
          <div>
            <h4 className="inspector-title">{adapterFile.split('/').pop()}</h4>
            <div dangerouslySetInnerHTML={{ __html: highlightPy(stadium_project[adapterFile]) }} />
          </div>
        );
      }
    ...
    if (inspectorTab === "hooks") {
      ...
      if (hooksDone.length === 0) {
        return <div className="inspector-empty">No hooks have fired yet. Run the simulation with a plugin-enabled client (stadium_project or Metaverse).</div>;
      }
      return (
        <div>
