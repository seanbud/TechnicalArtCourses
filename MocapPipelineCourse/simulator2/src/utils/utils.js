// ═══════════════════════════════════════════
// utils.js — Syntax highlighting & JSON tree (ported to ES modules)
// ═══════════════════════════════════════════
...
export function highlightPy(code) {
  var tokens = [];
  var src = code;
  var out = '';
  var i = 0;
...
export function buildJsonTreeHTML(obj, depth, oldObj) {
  depth = depth || 0;
  ...
  html += '<span class="jtoggle" onclick="window.__toggleJson(\'' + id + '\', event)">' + (expanded ? '▾' : '▸') + '</span>';
  ...
  entries.forEach(function(pair, i) {
    ...
    html += buildJsonTreeHTML(v, depth + 1, (oldObj && oldObj[k] !== undefined) ? oldObj[k] : undefined);
    if (i < len - 1) html += ',';
    html += '<br>';
  });
...
// Mount global toggle for inline onclick handlers in the JSON tree
if (typeof window !== 'undefined') {
  window.__toggleJson = function(id, e) {
